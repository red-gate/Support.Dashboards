...
SET ANSI_NULLS ON
GO
CREATE FUNCTION [data].[Cluster_SqlServer_Database_IntegrityCheck_StableSamples_DateRange]
(
    @MinDate AS BIGINT
    , @MaxDate AS BIGINT
    , @MaxSamples AS INT
...
, [Leaf].[_LastIntegrityCheck] AS [Cluster_SqlServer_Database_IntegrityCheck_LastIntegrityCheck]
, [utils].[TicksToDateTime]([Leaf].[_LastIntegrityCheck]) AS [Cluster_SqlServer_Database_IntegrityCheck_LastIntegrityCheck_DateTime]
FROM
 [SightedInstances]
LEFT OUTER JOIN
(
    SELECT [Id], MAX([CollectionDate]) as [GreatestMinDate]
    FROM [data].[Cluster_SqlServer_Database_IntegrityCheck_StableSamples]
...
INNER JOIN
 [data].[Cluster_SqlServer_Database_IntegrityCheck_StableSamples] [Leaf]
