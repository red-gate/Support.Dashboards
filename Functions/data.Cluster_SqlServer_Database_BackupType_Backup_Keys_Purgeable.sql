...
SET ANSI_NULLS ON
GO
CREATE FUNCTION [data].[Cluster_SqlServer_Database_BackupType_Backup_Keys_Purgeable]
(
    @PurgeDate AS BIGINT
)
RETURNS TABLE
...
FROM
    [data].[Cluster_SqlServer_Database_BackupType_Backup_Keys]
WHERE
...
(
    SELECT
        [Id]
    FROM
        [data].[Cluster_SqlServer_Database_BackupType_Backup_Sightings]
    WHERE
