...
SET ANSI_NULLS ON
GO
CREATE FUNCTION [data].[Cluster_Machine_LogicalDisk_Capacity_StableSamples_InstanceDateRange]
(
...
AS
RETURN
WITH [SightedInstances]([Id], [ParentId], [IdCollectionDate], [Cluster_Name], [Cluster_Machine_Name], [Cluster_Machine_LogicalDisk_Name]) AS
(
    ...
    INNER JOIN [data].[Cluster_Machine_LogicalDisk_Keys] ON [data].[Cluster_Machine_LogicalDisk_Keys].[ParentId] = [data].[Cluster_Machine_Keys].[Id]
     ...
     AND [data].[Cluster_Machine_LogicalDisk_Keys].[_Name] = @Cluster_Machine_LogicalDisk_Name
)
SELECT
[SightedInstances].[Id]
...
, [Leaf].[_TotalBytes] AS [Cluster_Machine_LogicalDisk_Capacity_TotalBytes]
FROM
 [SightedInstances]
LEFT OUTER JOIN
(
    SELECT [Id], MAX([CollectionDate]) as [GreatestMinDate]
    FROM [data].[Cluster_Machine_LogicalDisk_Capacity_StableSamples]
...
INNER JOIN
 [data].[Cluster_Machine_LogicalDisk_Capacity_StableSamples] [Leaf]
