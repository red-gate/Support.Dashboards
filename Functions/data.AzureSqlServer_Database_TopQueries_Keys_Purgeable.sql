...
SET ANSI_NULLS ON
GO
CREATE FUNCTION [data].[AzureSqlServer_Database_TopQueries_Keys_Purgeable]
(
    @PurgeDate AS BIGINT
)
RETURNS TABLE
...
FROM
    [data].[AzureSqlServer_Database_TopQueries_Keys]
WHERE
...
(
    SELECT
        [Id]
    FROM
        [data].[AzureSqlServer_Database_TopQueries_Sightings]
    WHERE
