...
SET ANSI_NULLS ON
GO
CREATE FUNCTION [data].[Cluster_ResourceGroup_Keys_Purgeable]
(
    @PurgeDate AS BIGINT
)
RETURNS TABLE
...
SELECT
    [data].[Cluster_ResourceGroup_Keys].[Id]
FROM
    [data].[Cluster_ResourceGroup_Keys]
WHERE
...
(
    SELECT
        [Id]
    FROM
        [data].[Cluster_ResourceGroup_Sightings]
    WHERE
