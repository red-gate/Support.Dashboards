...
SET ANSI_NULLS ON
GO
CREATE FUNCTION [data].[Cluster_Machine_Services_StableSamples_DateRange]
(
    @MinDate AS BIGINT
    , @MaxDate AS BIGINT
    , @MaxSamples AS INT
...
(
    ...
    WHERE [data].[Cluster_Machine_Services_Sightings].[SightingDate] >= @MinDate AND [data].[Cluster_Machine_Services_Sightings].[SightingDate] <= @MaxDate
)
, [SightedInstances]([Id], [ParentId], [IdCollectionDate], [Cluster_Name], [Cluster_Machine_Name], [Cluster_Machine_Services_ServiceName]) AS
(
    ...
    FROM [SightedIds]
     INNER JOIN [data].[Cluster_Machine_Services_Keys] ON [data].[Cluster_Machine_Services_Keys].[Id] = [SightedIds].[Id]
