...
SET ANSI_NULLS ON
GO
CREATE FUNCTION [data].[Cluster_SqlServer_ServerWaits_UnstableSamples_InstanceDateRange]
(
...
AS
RETURN
WITH [SightedInstances]([Id], [ParentId], [IdCollectionDate], [Cluster_Name], [Cluster_SqlServer_Name], [Cluster_SqlServer_ServerWaits_WaitType]) AS
(
    ...
    INNER JOIN [data].[Cluster_SqlServer_ServerWaits_Keys] ON [data].[Cluster_SqlServer_ServerWaits_Keys].[ParentId] = [data].[Cluster_SqlServer_Keys].[Id]
     ...
     AND [data].[Cluster_SqlServer_ServerWaits_Keys].[_WaitType] = @Cluster_SqlServer_ServerWaits_WaitType
)
SELECT
[SightedInstances].[Id]
