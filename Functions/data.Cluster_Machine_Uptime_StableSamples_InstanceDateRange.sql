...
SET ANSI_NULLS ON
GO
CREATE FUNCTION [data].[Cluster_Machine_Uptime_StableSamples_InstanceDateRange]
(
...
, [Leaf].[_StartDate] AS [Cluster_Machine_Uptime_StartDate]
, [utils].[TicksToDateTime]([Leaf].[_StartDate]) AS [Cluster_Machine_Uptime_StartDate_DateTime]
FROM
 [SightedInstances]
LEFT OUTER JOIN
(
    SELECT [Id], MAX([CollectionDate]) as [GreatestMinDate]
    FROM [data].[Cluster_Machine_Uptime_StableSamples]
...
INNER JOIN
 [data].[Cluster_Machine_Uptime_StableSamples] [Leaf]
