...
SET ANSI_NULLS ON
GO
CREATE FUNCTION [data].[Cluster_SqlServer_Services_Keys_InstanceDateRange]
(
...
AS
RETURN
WITH [SightedInstances]([Id], [ParentId], [IdCollectionDate], [Cluster_Name], [Cluster_SqlServer_Name], [Cluster_SqlServer_Services_ServiceName]) AS
(
    ...
    INNER JOIN [data].[Cluster_SqlServer_Services_Keys] ON [data].[Cluster_SqlServer_Services_Keys].[ParentId] = [data].[Cluster_SqlServer_Keys].[Id]
     ...
     AND [data].[Cluster_SqlServer_Services_Keys].[_ServiceName] = @Cluster_SqlServer_Services_ServiceName
)
SELECT
[SightedInstances].[Id]
